...
// import { useAuth } from "../context/AuthContext.jsx";
// import { useSocket } from "../context/SocketContext.jsx";
// import { FaPaperPlane, FaUserCircle, FaTimes, FaSpinner, FaSmile } from "react-icons/fa";
...
//             socket.on('dm:new', dmHandler);
//             socket.on('dm:reaction:added', reactionHandler);
//             socket.on('dm:reaction:removed', reactionHandler);
//             return () => {
//                 socket.off('dm:new', dmHandler);
//                 socket.off('dm:reaction:added', reactionHandler);
//                 socket.off('dm:reaction:removed', reactionHandler);
//             };
//         }
...
//         try {
//             await axiosInstance.post(`/api/messages/direct/${messageId}/react`, { emoji });
//         } catch (err) {
//             if (err.response?.status === 400) { // If user already reacted, remove it
//                 await axiosInstance.delete(`/api/messages/direct/${messageId}/react`, { data: { emoji } });
//             } else {
...
//                             <div className="flex-1 overflow-y-auto p-4 bg-gray-700 rounded-lg space-y-4">
//                                 {messages.map(msg => {
//                                     const isOwnMessage = msg.sender.toString() === user._id.toString();
//                                     const grouped = groupReactions(msg.reactions || []);

//                                     return (
//                                         <div
//                                             key={msg._id}
//                                             className={`flex relative group ${isOwnMessage ? 'flex-row-reverse' : 'flex-row'}`}
//                                         >
//                                             <div
...
//                                                 )}
//                                             </div>
//                                             <div className={`relative ${isOwnMessage ? 'ml-2' : 'mr-2'}`}>
//                                                 <button
...
//                                                     <div className={`absolute z-20 top-0 ${isOwnMessage ? 'right-full mr-2' : 'left-full ml-2'} bg-gray-700 p-2 rounded-lg shadow-lg flex space-x-1`}>
